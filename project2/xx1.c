...
#define BBSIZE 10	// Block buffer size - 10
...
struct bounded_buffer {
    ...
    void   *buffer[BBSIZE];    /* storage for the actual stuff held */
};
...
int nd_createBB(BoundedBuffer *newBB, int size) {
...
void init_network_driver(NetworkDevice *nd, void *mem_start,
    ...
    *fpds = create_fpds();
    ...
    int val = nd_createBB(&sendBB, BBSIZE);
    if (val == -1) {
    ...
    int i;
    for (i = 0; i <= MAX_PID; ++i) {
        val = nd_createBB(&recvBB[i], BBSIZE);
        if (val == -1) {
